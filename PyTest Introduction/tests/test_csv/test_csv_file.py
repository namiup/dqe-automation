...
def test_validate_schema(file_path):
...
@pytest.mark.validate_csv
@pytest.mark.skip
def test_age_column_valid(file_path):
...
@pytest.mark.validate_csv
def test_email_column_valid(file_path):
...
def test_active_players(id, is_active, file_path):
    found_id = False
    with open(file_path, newline='') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
