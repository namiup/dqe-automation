...
Compare HTML and Parquet Data
    ...
    ${html_data}=    Read Html File    ${html_path}

    # Step 2: Read Parquet data
    ${parquet_data}=    Read Parquet File    ${PARQUET_FOLDER}    ${DATE_COLUMN}    ${START_DATE}    ${END_DATE}

    # Step 3: Compare DataFrames
    ${comparison_result}=    Compare Html And Parquet    ${html_data}    ${parquet_data}
    ...
    Log    ${comparison_result['unmatched_html']}
    Log    ${comparison_result['unmatched_parquet']}
